...
htmlText.push('<li><a href="#" onmouseover="easytabs(\'1\', \''+i+'\');" onfocus="easytabs(\'1\', \''+i+'\');" onclick="return false;"  title="" id="tablink'+i+'">'+item+'</a></li>');
...
var req="Request="+$F('Request')+"&startPosition="+$F('startPosition')+"&maxRecords="+$F('maxRecords')+"&any="+$F('any')+"&bbox="+$F('bbox')+"&description="+$F('description')+"&organization="+$F('organization')+"&outputFormat="+$F('outputFormat')+"&subject="+$F('subject')+"&title="+$F('title')+"&language="+$F('language')+"&";
...
var htmlText=new Array();
	...
	if(candidates >1)	{				
						...
						htmlText.push('<td><a href="'+ct.urlcatalog+'?request=GetRecordById&elementSetName=full&outputFormat=text/xml&id='+json.GetRecordsResponse.Record[i].identifier+'" target="_blank">View metadata</a></td>');
						htmlText.push('</tr>');
	...
	var req=json.QueryString+"&";					
						...
						htmlText.push('<td><a href="'+ct.urlcatalog+'?request=GetRecordById&elementSetName=full&outputFormat=text/xml&id='+json.GetRecordsResponse.Record.identifier+'" target="_blank">View metadata</a></td>');
						htmlText.push('</tr>');
